...
plot(computeALE(svm.mod.classif, spam.gbm, "charExclamation"))
plot(intame(svm.mod.classif, spam.gbm, "charExclamation", fe_method = "PD",
  greedy = TRUE))
plot(intame(gbm.mod.classif, spam.gbm, "make", greedy = TRUE))
```
...
nnet.mod.classif = nnet(type ~ ., data = spam,  size = 10, linout = FALSE,
  decay=0.01, maxit = 1000, trace = FALSE)
nnet.mod.classif$
nnet.mod.classif$levels
tmp = predict(nnet.mod.classif, spam.gbm, probability = TRUE)
attributes(tmp)$probabilities[, "spam"]

plot(intame(nnet.mod.regr, boston, "lstat"))
plot(computeALE(nnet.mod.classif, spam.gbm, "charExclamation"))
plot(intame(nnet.mod.classif, spam.gbm, "charExclamation", fe_method = "PD",
  greedy = TRUE))
plot(intame(gbm.mod.classif, spam.gbm, "make", greedy = TRUE))
```
