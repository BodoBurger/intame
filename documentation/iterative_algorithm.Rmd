...
saf_x2 = iterative_partition(px2, pf2, "WMR2", threshold = .95, max_splits = 10,
  greedy = TRUE)
plot(saf_x2)
saf_x2
...
saf_x2_non_greedy = iterative_partition(px2, pf2, "WMR2", threshold = .95, max_splits = 10,
  greedy = FALSE)
...
ggplot() +
  geom_line(data = ale.x1$ale.plot.data, aes(x = fp_x, y = fp_f, col = "y_flat")) +
  geom_line(data = ale.x2$ale.plot.data, aes(x = fp_x, y = fp_f+5, col = "y_steep"))

th = sd(df$y) / 5 # threshold for L2
th = max(sd(ale.x1$fp_f), sd(ale.x2$fp_f)) / 5

saf_flat = iterative_partition(ale.x1$fp_x, ale.x1$fp_f, "WMR2")
saf_steep = iterative_partition(ale.x2$fp_x, ale.x2$fp_f, "WMR2")
...
ggplot() +
  geom_line(data = ale_x1_bc$ale.plot.data, aes(x = fp_x, y = fp_f, col = "y_flat")) +
  geom_line(data = ale_x2_bc$ale.plot.data, aes(x = fp_x, y = fp_f+5, col = "y_steep"))

th_bc = sd(df_bc$y_bc) / 5 # threshold for L2
th_bc = max(sd(ale_x1_bc$fp_f), sd(ale_x2_bc$fp_f)) / 5

saf_flat_bc = iterative_partition(ale_x1_bc$fp_x, ale_x1_bc$fp_f, "WMR2", .95)
saf_steep_bc = iterative_partition(ale_x2_bc$fp_x, ale_x2_bc$fp_f, "WMR2", .95)
