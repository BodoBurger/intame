...
saf_x2 = split_and_fit(px2, pf2, "WMSR2", threshold = .95, max_splits = 20)
plot(saf_x2)
saf_x2

saf_x2.RSS = split_and_fit(px2, pf2, "WMRSS", threshold = .3, max_splits = 20)
plot(saf_x2.RSS)
saf_x2.RSS

saf_x2_non_greedy = split_and_fit(px2, pf2, "WMSR2", threshold = .95, max_splits = 20,
  greedy = FALSE)
...
saf_flat.RSS = split_and_fit(x1, y_flat(x1), "WMRSS", .1)
saf_steep.RSS = split_and_fit(x1, y_steep(x1), "WMRSS", .1)
...
saf_x1 = split_and_fit(gbm.ale1$fp_x, gbm.ale1$fp_f, threshold = .9)
plot(saf_x1)
saf_x1

saf_x2 = split_and_fit(px2, pf2, method = "WMSR2", threshold = .9, max_splits = 20)
plot(saf_x2)
saf_x2
```
...
library(profvis)
profvis(split_and_fit(x1, y_flat(x1), "WMR2"))
profvis(split_and_fit(x1, y_flat(x1), "WMRSS",threshold = .1, max_splits = 3, greedy = FALSE))

profvis(split_and_fit(px2, pf2, "WMSR2", threshold = .95, max_splits = 20,
  greedy = FALSE))

library(microbenchmark)
microbenchmark(
  split_and_fit(x1, y_flat(x1), "WMR2", max_splits = 2),
  split_and_fit(x1, y_flat(x1), "WMR2", max_splits = 2, greedy = FALSE),
  times = 5)
