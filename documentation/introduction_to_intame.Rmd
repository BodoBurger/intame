...
```{r feature-1}
ALE.x1 = computeALE(nnet.mod$learner.model, df, "x1", grid.size = 30)
plot(ALE.x1, derivative = FALSE)
mean(ALE.x1$fe_f)
PD.x1 = computePD(nnet.mod$learner.model, df, "x1", grid.size = 30, grid.method = "quantile")
plot(PD.x1)
mean(PD.x1$fe_f)
```
...
```{r feature-2}
ALE.x2 = computeALE(nnet.mod$learner.model, df, "x2", grid.size = 30)
plot(ALE.x2)
PD.x2 = computePD(nnet.mod$learner.model, df, "x2", grid.size = 30, grid.method = "quantile")
plot(PD.x2)
...
```{r fig.cap="Green: breaks based on PD. Blue: ALE."}
breaks.ALE.cluster = partition(ALE.x2$fe_x, ALE.x2$fe_f, 5, part.method = "cluster")
breaks.PD.cluster = partition(PD.x2$fe_x, PD.x2$fe_f, 5,  part.method = "cluster")
plot(ALE.x2) +
...
```{r}
AME.x2 = intame(nnet.mod$learner.model, df, "x2", breaks = breaks.ALE,
  method = "PDeriv")
AME.x2
plot(AME.x2)
```
...
AME.x2.add.break.point = intame(nnet.mod$learner.model, df, "x2",
  breaks = c(0.25, breaks.PD), method = "PDeriv")
...
```{r}
AME.x2.auto = intame(nnet.mod$learner.model, df, "x2", method = "PDeriv")
AME.x2.auto
plot(AME.x2.auto)
AME.x2.auto$bounds
```
...
par(mfrow = c(1,1))
y = y1(x1) + y2(x2) + y3(x3) + y4(x4) + y5(x5) + y6(x6) + y7(x7) + rnorm(n, 0, .05)
...
```{r}
plot(computeALE(lm.mod$learner.model, data = dt, "x2"))
plot(computeALE(nnet.mod$learner.model, data = dt, "x2"))
plot(computeALE(svm.mod$learner.model, data = dt, "x2"))
plot(computeALE(gbm.mod$learner.model, data = dt, "x2",
  predict.fun = function(object, newdata) predict(object, newdata, n.trees = ntrees)))
```

```{r}
plot(computePD(lm.mod$learner.model, data = dt, "x2"))
plot(computePD(nnet.mod$learner.model, data = dt, "x2"))
plot(computePD(svm.mod$learner.model, data = dt, "x2"))
plot(computePD(gbm.mod$learner.model, data = dt, "x2",
  predict.fun = function(object, newdata) predict(object, newdata, n.trees = ntrees)))
```
